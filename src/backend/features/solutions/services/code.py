...
async def create_code_solution(
    ...
    tests = await test_crud.get_tests_by_task_id(session, task.id)

    check_is_active(task.is_active)

    solution = await code_solution_crud.create_solution(session, solution_in)
    check_code(group, account, tests)
    return solution
