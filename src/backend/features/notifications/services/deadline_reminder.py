...
from features.notifications.services.notification import (
    send_notification,
...
async def check_deadline_notifications(
    session: AsyncSession, task: BaseTask, time_left: timedelta
):
    minutes_left = time_left.total_seconds() / 60

    if 60 * 24 <= minutes_left < 60 * 24 + 1:
        if not await was_notification_sent(
            session, task.id, NotificationTypeEnum.DEADLINE_24H
        ):
        ...
        if not await was_notification_sent(
            session, task.id, NotificationTypeEnum.DEADLINE_1H
        ):
            await send_deadline_notification(
                session,
                task,
        ...
        if not await was_notification_sent(
            session, task.id, NotificationTypeEnum.DEADLINE_OVERDUE
        ):
            await send_deadline_notification(
                session,
                task,
...
async def send_deadline_notification(
    ...
    message = (
        f"До дедлайна задачи «{task.title}» осталось {hours} часов"
        if hours > 0
