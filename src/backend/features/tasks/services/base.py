...
) -> BaseTaskReadWithCorrectness | BaseTaskReadWithSolutions:
...
async def get_tasks_in_module(
    ...
    solutions = await base_solution_crud.get_solutions_by_account_id_and_task_ids(
        session, account.id, [task.id for task in tasks]
    )

    return mapper.build_base_task_read_with_correctness_list(tasks, solutions)
...
async def get_user_tasks(
    ...
    solutions = await base_solution_crud.get_solutions_by_account_ids_and_task_ids(
